...
import { getSafeZones, BOARD_SIZE, PLAYER_PATHS, MAX_PATH_INDEX, BoardType } from '../../constants/board';
...
// ⚡ Bolt Optimization: Extracted the heavy, static SVG board background into a separate component
// wrapped in React.memo. This prevents the complex vector graphics (lines, gradients, crosses)
// from re-rendering on every frame when piece positions (gameState) change.
// Impact: Significantly reduces frame drops and jank during piece movement animations.
const BoardBackground = React.memo(({ boardSize, cellSize, boardType }: { boardSize: number, cellSize: number, boardType: BoardType }) => {
