...
const Board: React.FC<BoardProps> = ({ gameState, onPiecePress }) => {
    ...
    // Memoize the renderSafeZoneCross function so it can be used safely in useMemo without warning
    const renderSafeZoneCrossMemoized = React.useCallback((index: number) => {
    ...
    }, [cellSize, getCellCenter]);

    // Memoize the SVG background to prevent re-rendering grid lines and safe zones on every animation step
    const svgBackground = React.useMemo(() => (
        <Svg height="100%" width="100%" style={styles.svgLayer}>
            ...
            {getSafeZones(gameState.boardType).map((safeIndex: number) => renderSafeZoneCrossMemoized(safeIndex))}
        </Svg>
    ), [cellSize, boardSize, gameState.boardType, renderSafeZoneCrossMemoized]);
