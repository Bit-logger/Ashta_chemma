...
// ⚡ Bolt Optimization: Extracted and memoized the static SVG background.
// This prevents the complex ground texture, 5x5 grid lines, and safe zone crosses
// from re-rendering every 200ms when game pieces animate their moves.
const BoardBackground = React.memo(function BoardBackground({ boardSize, boardType }: { boardSize: number, boardType: BoardType }) {
...
const Board: React.FC<BoardProps> = ({ gameState, onPiecePress }) => {
    ...
    return (
        <View style={[styles.container, { width: boardSize, height: boardSize }, styles.boardMargin]}>
            <BoardBackground boardSize={boardSize} boardType={gameState.boardType} />
