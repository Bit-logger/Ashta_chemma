...
// Memoized static background to prevent re-rendering the complex SVG on every piece move
const StaticBoardBackground = React.memo(({ boardSize, cellSize, boardType }: StaticBoardBackgroundProps) => {
    // Helper to get center coordinates of a specific cell (0-24)
    const getCellCenter = (index: number) => {
        const row = Math.floor(index / BOARD_SIZE);
        const col = index % BOARD_SIZE;
