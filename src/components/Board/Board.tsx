...
const Board: React.FC<BoardProps> = ({ gameState, onPiecePress }) => {
    ...
    const renderSafeZoneCross = React.useCallback((index: number) => {
    ...
    ), [boardSize, cellSize, gameState.boardType, renderSafeZoneCross]);

    // Pre-calculate physical cell occupancies to avoid O(N^2) loop during render
    const cellOccupancyMap = React.useMemo(() => {
        const map = new Map<number, string[]>();
        gameState.players.forEach(p => {
            p.pieces.forEach(piece => {
                if (piece.position !== -1 && piece.position <= MAX_PATH_INDEX) {
                    const cellIndex = PLAYER_PATHS[p.id][piece.position];
                    if (!map.has(cellIndex)) {
                        map.set(cellIndex, []);
                    }
                    map.get(cellIndex)!.push(piece.id);
                }
            });
        });
        return map;
                ...
                .map(player =>
                    player.pieces.map((piece, index) => {
                        ...
                        if (piece.position === -1) {
                            ...
                            const occupants = cellOccupancyMap.get(cellIndex);
                            if (occupants) {
                                stackIndex = occupants.filter(id => id < piece.id).length;
                            }
