...
const renderSafeZoneCross = (index: number, cellSize: number) => {
...
// ⚡ Bolt Optimization: Memoize the complex SVG background to prevent expensive
// re-renders on every frame during piece movement animations.
const BoardBackground = React.memo<BoardBackgroundProps>(({ boardSize, cellSize, boardType }) => {
    return (
        <Svg height="100%" width="100%" style={styles.svgLayer}>
            ...
            {getSafeZones(boardType).map((safeIndex: number) => renderSafeZoneCross(safeIndex, cellSize))}
        </Svg>
    );
});
