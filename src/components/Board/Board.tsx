...
const BoardBackgroundComponent: React.FC<BoardBackgroundProps> = ({ boardSize, cellSize, boardType }) => {
    // ⚡ Bolt Optimization: Extracted the heavy, static SVG elements (the board background,
    // grid lines, and safe zone crosses) into a separate component. When wrapped in React.memo,
    // this prevents the SVG engine from continually reconciling this complex tree on every
    // tick when player pieces animate or change position, resulting in much smoother frame rates.
