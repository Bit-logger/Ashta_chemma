...
type BoardBackgroundProps = {
    boardSize: number;
    cellSize: number;
    boardType: string | number; // Assuming type based on getSafeZones signature
};

// ⚡ Bolt Optimization: Extracted static SVG background elements into a React.memo wrapped component.
// This prevents expensive SVG re-renders when pieces animate and the gameState changes, significantly
// reducing frame drops during piece movement.
const BoardBackground = React.memo(({ boardSize, cellSize, boardType }: BoardBackgroundProps) => {
    ...
    return (
        <Svg height="100%" width="100%" style={styles.svgLayer}>
            ...
            {getSafeZones(boardType as any).map((safeIndex: number) => renderSafeZoneCross(safeIndex))}
        </Svg>
    );
});
